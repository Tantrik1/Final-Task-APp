-- Fix trigger_push_notification to check active subscriptions directly
-- instead of relying on the unreliable push_enabled flag
...
DECLARE
  ...
  v_task_id TEXT;
BEGIN
  -- Check if user has active push subscriptions (the only reliable check)
  SELECT EXISTS (
    SELECT 1 FROM public.push_subscriptions
    WHERE user_id = NEW.user_id AND is_active = true
  ) INTO v_has_subscription;
  
  -- Only proceed if user has active subscriptions
